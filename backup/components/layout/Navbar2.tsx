...
const Navbar2 = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-[#019445] border-b border-gray-200 font-serif ">
      <div className="max-w-[1440px] mx-auto flex items-center justify-between px-6 md:px-8 py-1">
        {/* Logo Section */}
        <Link href={"/"} className="flex items-center gap-2 bg-white p-1 rounded-sm flex-col">
        ...
        <div className="hidden lg:flex items-center gap-6 text-[13px] text-white">
          {categories.map((cat) => (
            <Link
              key={cat.name}
              href={cat.href}
        ...
        <div className="flex items-center gap-4">
          <Link
            href={"/contact"}
            className="hidden sm:block bg-black hover:bg-[#7cb140] text-white px-10 py-4 text-[10px] uppercase tracking-widest transition-all duration-300"
          >
             Contact Us
          </Link>
